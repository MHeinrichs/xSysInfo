...
int Dhry_Initialize(void)
{
  ...
  //memset does only clear the first line
  for(int i=0; i<50; ++i){
    Arr_1_Glob[i]=0;
    for(int j=0; j<50;++j){
      Arr_2_Glob[i][j]=0;  
...
void Dhry_Run(unsigned long Number_Of_Runs)
{
  ...
  Runs_To_Do = Number_Of_Runs;
  if (Runs_To_Do > (unsigned long)INT_MAX)
    Runs_To_Do = (unsigned long)INT_MAX;

  for (Run_Index = 1; Run_Index <= Runs_To_Do; ++Run_Index)
  {
